...
parser.add_argument(
    "--raf_path", type=str, default="../raf-basic", help="raf_dataset_path"
)
parser.add_argument(
    "--resnet50_path",
    type=str,
    default="../model/resnet50_ft_weight.pkl",
    help="pretrained_backbone_path",
)
parser.add_argument(
    "--label_path", type=str, default="list_patition_label.txt", help="label_path"
)
...
parser.add_argument(
    ...
    type=float,
    default=0.3,
    help="ratio of label shuffle in training only 0,0.1,0.2,0.3",
)
...
def train(args, model, train_loader, optimizer, scheduler, device):
    ...
    for batch_i, (imgs1, labels, indexes, imgs2, _) in enumerate(train_loader):
        ...
        running_loss += loss
        ce_loss_s += loss1
        flip_loss_s += flip_loss_l

    scheduler.step()

    running_loss = running_loss / iter_cnt
    ce_loss_avg = ce_loss_s / iter_cnt
    flip_loss_avg = flip_loss_s / iter_cnt
    acc = correct_sum.float() / float(train_loader.dataset.__len__())
    return acc, running_loss, ce_loss_avg, flip_loss_avg
...
def find_high_flip_loss_images(args, model, data_loader, device):
    ...
    with torch.no_grad():
        for (
            ...
            imgs2,
            img_paths,
        ) in data_loader:  # Ensure img_paths is used
            imgs1 = imgs1.to(device)
            imgs2 = imgs2.to(device)
            labels = labels.to(device)
            ...
            for i in range(len(labels)):
                ...
                loss = ACLoss(
                    hm1_single, hm2_single, grid_l, output[i].unsqueeze(0)
                )  # scalar
    ...
    return {"high_loss_images": high_loss_images, "mean_losses": mean_losses}
...
def main():
    ...
    train_dataset = FER2013Dataset(
        root_dir=args.raf_path, phase="train", transform=train_transforms
    )
    val_dataset = FER2013Dataset(
        root_dir=args.raf_path, phase="val", transform=eval_transforms
    )
    test_dataset = FER2013Dataset(
        root_dir=args.raf_path, phase="test", transform=eval_transforms
    )
    ...
    run = wandb.init(
        project="fer2013-eac",
        name="rebuttal_50_noise_" + str(args.label_path),
        config={
            "batch_size": args.batch_size,
            "epochs": args.epochs,
            "lr": 0.0001,
    ...
    for i in range(1, args.epochs + 1):
        train_acc, train_loss, ce_loss, flip_loss = train(
        ...
        if val_loss < best_loss:
            ...
            torch.save(
                model.state_dict(),
                "best_loss_res_50_13_" + str(args.label_path) + ".pth",
            )
        wandb.log(
            {
                "Epoch": i,
    ...
    wandb.log(
        {"Confusion Matrix": [wandb.Image("conf_mat.png", caption="Confusion Matrix")]}
    )
